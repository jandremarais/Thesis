...
\end{cases}
$$
Thus the cross-entropy penalises $p_{ij}$ close to zero when $y_{ij}=1$ (label $j$ is present) and $p_{ij}$ near 1 when $y_{ij}=0$. When training the network the cross-entropy is summed over all labels and averaged over the data points in a batch to get the loss for that batch:

$$
L=\frac{1}{n}\sum_{i=1}^{n}\sum_{j=1}^{K}\text{CE}(p_{ij},y_{ij})
$$
...
The idea is that the loss contribution for a label pair should have a greater weight if the 

+ LSEP (very similar to BP-MLL but with a bias term and negative sampling)
