...
Some of the reasons why it may be hard to distinguish the signal from the noise in the X-ray images as their raw form is as follows (adapted from [@Karpathy2015]):
...
$$
\text{LDens}(D)=\frac{1}{K}\text{LCard}(D)=\frac{1}{NK}\sum_{i=1}^{N}\sum_{k=1}^{K}y_{ik}.
$$
...
$$
\text{PLDiv}(D)=\frac{1}{N}\text{LDiv}(D).
$$
...
```{r, include=FALSE, eval=FALSE}
good_img <- image_read('~/Documents/ChestXray8/data/00000900_001.png')
bad_img <- image_read('~/Documents/ChestXray8/data/00001200_009.png')
...
There are three possible aggregation schemes: per *example*, *micro*- and *macro*-averaging. In some literature [@Zhang2014], micro- and macro-averaging are grouped into *label*-based averaging, however we find the example-micro-macro grouping more intuitive.

Suppose a binary performance metric comparing (dis)similarity between two binary vectors is denoted as $B$, *i.e.* $B\in\{\text{accuracy},\text{precision},\text{recall},F_{1},\text{AUC}\}$. Example-based multi-label evaluation metrics can then be given as:
...
perf1 <- mldr_evaluate(D, f1)[c("HammingLoss", "SubsetAccuracy", "AveragePrecision",
                       "MicroFMeasure", "MacroFMeasure")] %>% unlist %>% round(4)
perf2 <- mldr_evaluate(D, f2)[c("HammingLoss", "SubsetAccuracy", "AveragePrecision",
                       "MicroFMeasure", "MacroFMeasure")] %>% unlist %>% round(4)
perf3 <- mldr_evaluate(D, f3)[c("HammingLoss", "SubsetAccuracy", "AveragePrecision",
                       "MicroFMeasure", "MacroFMeasure")] %>% unlist %>% round(4)
...
df_print[1, df[1, ] == min(df[1, ])] <- paste0("**", df[1, df[1, ] == min(df[1, ])], "**")
df_print[2, df[2, ] == max(df[2, ])] <- paste0("**", df[2, df[2, ] == max(df[2, ])], "**")
df_print[3, df[3, ] == max(df[3, ])] <- paste0("**", df[3, df[3, ] == max(df[3, ])], "**")
df_print[4, df[4, ] == max(df[4, ])] <- paste0("**", df[4, df[4, ] == max(df[4, ])], "**")
df_print[5, df[5, ] == max(df[5, ])] <- paste0("**", df[5, df[5, ] == max(df[5, ])], "**")

kable(df_print, booktabs = TRUE, escape = FALSE, digits = 4,
...
+ $_{1}\hat{h}$ and $_{3}\hat{h}$ is equivalent in terms of both hamming loss and subset accuracy, whereas $_{3}\hat{h}$ has the better $AP^{example}$ and $_{1}\hat{h}$ the better $F_{1}^{micro}$ and $F_{1}^{macro}$.
+ $F_{1}^{micro}$ and $F_{1}^{macro}$ measure different aspects of the predictions.
