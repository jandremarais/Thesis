...
1. Image classification is a highly relevant topic in computer vision and machine (and statistical) learning. It is a thoroughly researched domain and already by many regarded as a 'solved' problem. This progress is mainly attributed to the yearly large-scale image classification competition, *ImageNet*[^imagenet], and the development of *Convolutional Neural Networks* (CNNs). The last five (get the right number) winners of ImageNet all used a variant of CNNs in their solution. However, the main focus up until recently was on problems of single label classification. Therefore, the field of multi-label image classification is nowhere near the maturity level of its single-label counterpart. Multi-label classification has a wide range of applications, not only in image classification. It has been applied to problems in text categorisation, multimedia, biology, chemical data analysis, social network mining and e-learning among others. This is most likely the reason why it has seen such a rapid increase in interest in the literature (see \autoref{pubsperyear}). However, researchers have not yet reached consensus on how to deal with many of the aspects when learning from multi-labelled data, *e.g.* how to handle dependency between labels. There are a very limited number of publications specifically dealing with multi-label classification of images and using CNNs for this scenario. The field can gain from an up-to-date review of the literature, more statistical perspectives on some of the challenges, additional benchmark datasets and quality empirical evaluations of the theory.  
...
[^imagenet]: imagenet webiste link
...
This thesis works towards building a multi-label classifier that can label satellite images of the Amazon as accurately as possible. Since this is an image classification problem, it is reasonable to restrict the space of possible classifiers to CNNs. The method thought best to achieve this goal is to:
...
> should I have a section on contributions? what I will contribute to literature.
