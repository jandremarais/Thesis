...
+ In this work, we focus on multi-label image classification methods with deep learning techniques.
+ Every real-world image can be annotated with multiple labels, because an image normally abounds with rich semantic information, such as objects, parts, scenes, actions, and their interactions or attribute 
+ transfer learning with single label networks not optimal (see CNN: Single-label to Multi-label)
+ labels may relate to different visual regions over the whole image (SRN paper intro). Proposal based approaches attempt to solve this, but they ignore semantic label relations.
+ The diverse and complex contents in multi-label images make it difficult to learn effective feature representations and classifiers.
+ important task in computer vision with various applications, such as scene recognition, multi-object recognition, human attribute recognition, etc.
+ not well studied compared to single-label. Also more challenging and practical (more natural problem).
+ ml loss (for ml output and label dependence)
+ proposal based methods to accommodate that objects relate to different regions in an image
+ model semantic relations, with graphical models, RNNs, structured inference
+ model spatial relations, with SRN. Usually not possible because need annotations. (attention mechanism important here)
...
[@Zhu2017a] used multi-label CNN for pedestrian attribute detection. The adaption they made to the CNN was also by redefining the loss function. The defined the loss function as
...
However, these methods do not capture spatial relations between labels. This is a challenging problem because most of the times these spatial locations are not known beforehand, *i.e* the images are not annotated with these spatial locations. Spatial Regularisation Networks [@Zhu2017] attempt to capture both semantic and spatial relations between labels without any prior knowledge on the spatial locations of each label. This is discussed in the next section.
